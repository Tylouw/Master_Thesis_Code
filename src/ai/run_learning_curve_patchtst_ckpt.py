...
def train_single_run(
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        va_acc = float(val_stats.get("acc", float("nan")))

        if print_epochs and (epoch % max(1, int(print_every)) == 0):
...
def save_checkpoint(path: Path, model: nn.Module, extra: Dict):
    """Save a portable checkpoint (model weights on CPU + metadata)."""
    path.parent.mkdir(parents=True, exist_ok=True)
    state_cpu = {k: v.detach().cpu() for k, v in model.state_dict().items()}
    ckpt = {"model_state_dict": state_cpu, **extra}
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument(
        "--print-epochs",
        action="store_true",
        help="Print per-epoch progress (fraction/rep + train/val loss/acc + epoch time).",
    )
    ap.add_argument(
        "--print-every",
        type=int,
        default=1,
        help="Print every N epochs when --print-epochs is set (default 1 = every epoch).",
    )

    ap.add_argument("--save-best", action="store_true", help="Save best (val-selected) checkpoint to out-dir")
    ap.add_argument(
        "--save-best-each-run",
        action="store_true",
        help="If set, save a checkpoint for every run in the learning curve / OOD loops (can be a lot of files)",
    )
    ap.add_argument("--ckpt-dir", type=str, default="checkpoints", help="Subfolder under out-dir to store checkpoints")
    ap.add_argument("--ckpt-prefix", type=str, default="patchtst", help="Checkpoint filename prefix")
    ...
    print(f"results_csv: {(out_dir / 'learning_curve_results.csv').resolve()}")
    print(f"ckpt_dir: {(out_dir / args.ckpt_dir).resolve()}  (save_best={args.save_best})")
    ...
    ckpt_dir = out_dir / str(args.ckpt_dir)
    if bool(args.save_best):
        ...
        for f in fractions:
            for rep in range(int(args.repeats)):
                ...
                run_tag = f"f={f:.2f} rep={rep}/{args.repeats-1}"
                model, tr_summary = train_single_run(
                    ...
                    run_tag=run_tag,
                    print_epochs=bool(args.print_epochs),
    ...
    # Optionally save checkpoint (the model is already the best epoch on val)
    if bool(args.save_best):
        should_save = bool(args.save_best_each_run) or (abs(float(f) - float(max_fraction)) < 1e-9 and int(rep) == 0)
        if should_save:
            ckpt_path = ckpt_dir / f"{args.ckpt_prefix}_iid_f{float(f):.2f}_rep{int(rep)}_seed{int(run_seed)}.pt"
            payload = {
                "phase": "iid",
                ...
                "model_preset": args.model_preset,
                "select_metric": args.select_metric,
                "train_summary": tr_summary,
                "test_loss": float(test_loss),
                "test_stats": dict(test_stats),
            }
            save_checkpoint(ckpt_path, model, extra=payload)
            # Convenience pointer: always overwrite a "best.pt" with the last saved checkpoint
            save_checkpoint(ckpt_dir / f"{args.ckpt_prefix}_best.pt", model, extra=payload)
            print(f"Saved checkpoint: {ckpt_path}")
            print(f"Updated best pointer: {ckpt_dir / (args.ckpt_prefix + '_best.pt')}")
    ...
    if args.ood_key.strip():
        ...
        # Optionally save checkpoint for this OOD run
        if bool(args.save_best):
            should_save = bool(args.save_best_each_run) or (int(rep) == 0)
            if should_save:
                safe_holdout = str(holdout).replace("/", "_").replace(" ", "_")
                ckpt_path = ckpt_dir / f"{args.ckpt_prefix}_ood_{ood_key}_{safe_holdout}_rep{int(rep)}_seed{int(run_seed)}.pt"
                save_checkpoint(
                    ckpt_path,
                    model,
                    extra={
                        ...
                        "model_preset": args.model_preset,
                        "select_metric": args.select_metric,
                        "train_summary": _tr_summary,
                        "ood_loss": float(ood_loss),
                        "ood_stats": dict(ood_stats),
                    },
                )
